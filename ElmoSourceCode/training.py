...
class LanguageModel(object):
    ...
    '''
    def __init__(self, dimNum,options, is_training):
        self.dimNum=dimNum
    ...
    def _build_word_embeddings(self):
        ...
        with tf.device("/cpu:0"):
            ...
            ############################################################
            #这一段参考https://blog.csdn.net/qq_27009517/article/details/85089244
            #embedding_addr='../CharEmbedding/DaGuan/DaGuanGloveForElmo.txt'
            embedding_addr=''
            if self.dimNum==150:
                embedding_addr='inputs/embedding_matrix/150dim/DaGuanGloveForElmo_150dim.txt'
            elif self.dimNum==200:
                embedding_addr='inputs/embedding_matrix/200dim/DaGuanGloveForElmo_200dim.txt'
            elif self.dimNum==250:
                embedding_addr='inputs/embedding_matrix/250dim/DaGuanGloveForElmo_250dim.txt'
            elif self.dimNum==300:
                embedding_addr='inputs/embedding_matrix/300dim/DaGuanGloveForElmo_300dim.txt'
            ...
            for i, word in enumerate(words):
                tmp_embed[i+3][:]=w2v_model[word] #从3开始是因为前面3个是<S>,</S>,<UNK>
            ################################################################
        
            self.embedding_weights = tf.get_variable(
                "embedding", [n_tokens_vocab, projection_dim],
                dtype=DTYPE, initializer=tf.constant_initializer(tmp_embed),  #这里加了initializer和trainable
                trainable=True
            )
...
def train(dimNum,options, data, n_gpus, tf_save_dir, tf_log_dir,
        ...
        for k in range(n_gpus):
            with tf.device('/gpu:%d' % k):
                with tf.variable_scope('lm', reuse=k > 0):
                    ...
                    #   lstm states
                    model = LanguageModel(dimNum,options, True)
    ...
    with tf.Session(config=tf.ConfigProto(
        ...
        for batch_no, batch in enumerate(data_gen, start=1):
            ...
            if batch_no == n_batches_total:
                ...
                #with h5py.File('../CharEmbedding/DaGuan/DaGuanElmo.hdf5','w') as fout:
                    #fout.create_dataset('embedding',embed.shape,dtype='float32',data=embed)
                savePath=''
                if dimNum==150:
                    savePath='ELMo/150dim/DaGuanElmo_150dim.hdf5'
                elif dimNum==200:
                    savePath='ELMo/200dim/DaGuanElmo_200dim.hdf5'
                elif dimNum==250:
                    savePath='ELMo/250dim/DaGuanElmo_250dim.hdf5'
                elif dimNum==300:
                    savePath='ELMo/300dim/DaGuanElmo_300dim.hdf5'
