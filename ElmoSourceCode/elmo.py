...
def weight_layers(name, bilm_ops, l2_coef=None,
    ...
    lm_embeddings = bilm_ops['lm_embeddings']  #[None,3,None,1024]
    mask = bilm_ops['mask']
    ...
    with tf.control_dependencies([lm_embeddings, mask]):
        ...
        if use_top_only:
            ...
            # split LM layers
            layers = tf.split(lm_embeddings, n_lm_layers, axis=1)  #3个[None,1,None,200] projection的维数100乘以2
    
            # compute the weighted, normalized LM activations
            pieces = []
            for w, t in zip(normed_weights, layers):
                if do_layer_norm:
                    pieces.append(w * _do_ln(tf.squeeze(t, squeeze_dims=1))) #每个[None,1,None,1024]squeeze后，变成[None,None,200]
                else:
                    pieces.append(w * tf.squeeze(t, squeeze_dims=1))  #相乘后放入list，得到的是3个[None,None,200]的list
            sum_pieces = tf.add_n(pieces) #3个[None,None,200]对应相加，结果还是[None,None,200]，相当于加权后相加
