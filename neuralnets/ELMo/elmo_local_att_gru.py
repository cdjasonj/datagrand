...
class   Local_att_gru:
    ...
    def build_model(self):
        ...
        for size in self.params['LSTM-Size']:
            shared_layer = Bidirectional(CuDNNGRU(size,return_sequences=True))(shared_layer)
            shared_layer = Dropout(self.params['dropout'])(shared_layer)
            local_att = SeqSelfAttention(attention_width=15, attention_activation='sigmoid' )(shared_layer)
            shared_layer = Concatenate(axis=-1)([shared_layer,local_att])
