...
class   Bilstm_cnn_crf:
    def __init__(self, char_word2vec, char_glove, char_fasttext,bichar_word2vec,bichar_glove,bichar_fasttext
        ...
        defaultParams = {'dropout': 0.25,'l2_rate':1e-3,'hidden_size':450,
    ...
    def build_model(self):
        ...
        shared_layer = Bidirectional(CuDNNLSTM(self.params['hidden_size'],return_sequences=True))(shared_layer)
        shared_layer = Dropout(0.25)(shared_layer)
        shared_layer = Conv1D(filters=self.params['hidden_size'],kernel_size=3,  padding='same', activation='relu')(shared_layer)
        shared_layer = Dropout(0.25)(shared_layer)
        shared_layer = Conv1D(filters=self.params['hidden_size'], kernel_size=5, padding='same',activation='relu')(shared_layer)
        shared_layer = Dropout(0.25)(shared_layer)
        shared_layer = Conv1D(filters=self.params['hidden_size'], kernel_size=7, padding='same',activation='relu')(shared_layer)
        shared_layer = Dropout(0.25)(shared_layer)

        self_att = SeqSelfAttention()(shared_layer)
        shared_layer = Concatenate(axis=-1)([shared_layer,self_att])
